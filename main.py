...
from prod_help import endless_try, log_function
...
def reminder(queue):
    ...
    while True:
        ...
        while not queue.empty():
            ...
            try:
                queueWasEmpty = False
                logging.info("New messages of type: "+data['type'])

                if any( [data[key] == None for key in data.keys()] ):
                    logging.warning("FOUND AN INCOMPLETE ENTRY")
                    continue
                ...
                bot.send_message(data['chat_id'],
                                 f"Unexpected error: {data['type']} was maybe not handled correctly")
